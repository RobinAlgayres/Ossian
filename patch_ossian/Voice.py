...
### --------------------------------------------------------------------------------   
    ...
    def prep_utt_from_time_lab(self,utt,basename):
        target_nodes="//segment"
    	output_label_filetype="time_lab"
	silence_symbol="sil"
	min_silence_duration="50"
	target_attribute="pronunciation"
	silence_tag="has_silence"
	skippables=[c.POSS_PAUSE]

 	add_segments_and_states_from_label(utt, input_label=output_label_filetype,\
                                 target_nodes=target_nodes, \
                                 target_attribute=target_attribute, \
                                 skippables=skippables )  ##TODO: extend this to non-state case

        propagate_start_and_end_times_up_tree(utt) ## to get e.g. woken start and end etc.

        remove_short_silent_segments2(utt, target_nodes=target_nodes, \
                                        target_attribute=target_attribute, \
                                        silence_symbol=silence_symbol,  \
                                        min_silence_duration=min_silence_duration)

        propagate_silence_tag_up_tree(utt, target_nodes=target_nodes, silence_symbol=silence_symbol, \
                                        target_attribute=target_attribute, \
                                        output_attribute=silence_tag)
	...
	time_lab_location=self.res.make_dir(c.VOICE, "output/time_lab")
...
#            os.remove(utterance_path)        
        ...
        if self.make_archive:
            utt.archive()
	utt.pretty_print()
	#print(input_string,output_labfile,output_uttfile)
        for processor in self.processors:
