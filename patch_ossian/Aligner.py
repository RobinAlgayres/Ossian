...
class SegmentAligner(SUtteranceProcessor):
    ...
    def __init__(self, processor_name='aligner', target_nodes='//segment', target_attribute='segment_name', \
                    ...
                    training_settings={'MIXTURE_SCHEDULE': '0 0 0', 'NREEST': '2'}, n_training_utts=0,do_training_bool=True,src_time_lab=None,dest_time_lab=None):
...
#         _PUNC_ = sil, skip 
	...
	self.do_training_bool=do_training_bool
	...
	if not self.do_training_bool:
		if not os.path.isdir(dest_time_lab):
			shutil.copytree(src_time_lab,dest_time_lab)   
        super(SegmentAligner, self).__init__()
	

    def process_utterance(self, utt):

	if self.do_training_bool:
		if not self.trained:
        ...
        propagate_silence_tag_up_tree(utt, target_nodes=self.target_nodes, \
					silence_symbol=self.silence_symbol, \
                                        target_attribute=self.target_attribute, \
	...
	if not self.do_training_bool:
		self.trained=True
		print("Skipping Aligner training")
		return
...
#             
       ...
       remove_short_silent_segments2(utt, target_nodes=self.target_nodes, \
					target_attribute=self.target_attribute, \
                                        silence_symbol=self.silence_symbol,  \
                                        min_silence_duration=self.min_silence_duration)
